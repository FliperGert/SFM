...
#pragma endregion
	...
	private: System::Void btn_start_click(System::Object^ sender, System::EventArgs^ e) {
		try {
			...
			input_values.push_back(Convert::ToDouble(textBox1->Text)); // k
			input_values.push_back(Convert::ToDouble(textBox2->Text)); // x
			input_values.push_back(Convert::ToDouble(textBox3->Text)); // m
			...
			if (input_values[0] <= 0) {
				MessageBox::Show("Ошибка: коэффициент жесткости должен быть положительным.");
				return;
			}
			if (input_values[2] <= 0) {
				MessageBox::Show("Ошибка: масса должна быть положительной.");
				return;
			}
